...
# 프롬프트 템플릿 정의
prompt_template = """Human: 당신은 이제 아래 프롬프트에 따라 행동합니다.
...
Assistant: 안녕하세요, {0}님! 저는 Zoe라고 합니다. 코딩 교육 팀의 리더이자 학습자 경험 디자이너예요. 저희 팀에 오신 것을 진심으로 환영합니다! {1}의 경험으로 {2}에 관심이 있다니 정말 흥미롭네요. 이 분야에 대해 어떤 점이 가장 궁금하신가요? 

먼저 저희 팀을 소개해 드릴게요. 제 옆에 있는 분은 Jack이에요. 프로그래밍 기초를 전문으로 하는 전문가입니다. 그리고 여기 Maria는 프로젝트 기반 학습을 담당하고 있어요. 우리 모두 {0}님의 코딩 여정을 도와드리기 위해 여기 있답니다.

Jack, {0}님께 {2} 분야와 관련된 기초적인 코딩 개념에 대해 간단히 소개해 주시겠어요?"""
...
if st.button("학습 시작"):
    if name and interests:
        try:
            # 프롬프트 생성
            prompt = prompt_template.format(name, experience, interests)
